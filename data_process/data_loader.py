...
def get_select_idxs(total_time_len, train_len, embedding_len, getDataSkipStep, nb_samples=None):
    """
    ...
    :return:
    """
    max_nb_samples = (total_time_len - embedding_len - 1) // getDataSkipStep
    print(max_nb_samples)
    print(nb_samples)
    assert 0 < nb_samples < max_nb_samples, 'nb_samples is too large!'
    if nb_samples is not None:
        return np.array([train_len + i * getDataSkipStep for i in range(max_nb_samples)])[:nb_samples]
        # if getDataSkipStep == train_len:
        #     return np.array([(i+1) * getDataSkipStep for i in range(max_nb_samples)])[:nb_samples]
        # else:
        #     return np.array([train_len + getDataSkipStep for i in range(max_nb_samples)])[:nb_samples]
    else:
        # return np.array([(i + 1) * getDataSkipStep for i in range(max_nb_samples)])
        return np.array([train_len + i * getDataSkipStep for i in range(max_nb_samples)])

# def get_select_idxs_v2(total_time_len, train_len, embedding_len, nb_samples):
#     """
...
#     :return:
#     """
#     max_time_point = total_time_len - embedding_len - 1
#     # print(max_nb_samples)
#     # print(nb_samples)
#     # print(max_time_point)
#     # print(nb_samples)
#     assert 0 < train_len + nb_samples < max_time_point, 'nb_samples is too large!'
#     return np.array([train_len + i for i in range(nb_samples)])
